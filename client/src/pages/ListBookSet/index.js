...
function ListBookSet() {
  ...
  useEffect(() => {
    axios.get("https://fptu-library/api/book-sets/list")
      .then((response) => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`https://fptu-library/api/book-sets/delete/${id}`);
                ...
                {filteredBookSetData.map((bookSet) => (
                  <tr key={bookSet._id} className="align-middle">
                    <td>
                      {bookSet.image ? (
                        <img
                          src={`https://fptu-library/api/book-sets/image/${bookSet.image.split("/").pop()}`}
                          alt={bookSet.title}
