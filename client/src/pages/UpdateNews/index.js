...
function UpdateNews() {
  ...
  useEffect(() => {
    ...
    const fetchNewsDetail = async () => {
      try {
        const res = await axios.get(`https://fptu-library/api/news/get/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `https://fptu-library/api/news/update/${id}`,
        formData,
        {
          headers: {
          ...
          <div className="col-md-3">
            <div className="form-group">
              {thumbnail ? (
                ...
                <img
                  src={`https://fptu-library/api/news/thumbnail/${data.thumbnail
                    .split("/")
                    .pop()}`}
                  className="img-fluid"
                  alt={data.title}
